...
//'
//' @description
//' Calculates the probability of an unfavorable pair, defined as \eqn{P(X \le Y - \lambda)},
//' across a vector of thresholds \eqn{\lambda} (this is the approach used in BuyseTest). This represents the probability
...
//' @export
// [[Rcpp::export]]
 NumericVector c_loss_buyse(NumericVector x_uni, NumericVector x_w,
   ...
   for (int i = 0; i < n_lam; ++i) {
     ...
     NumericVector Fc = c_ecdf_values_less(y_uni, y_w, x_uni, -lambda[i]);
