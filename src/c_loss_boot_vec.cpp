...
//' Multivariate ID-Linked Bootstrap for Loss Probability (Survival & Non-Survival)
//'
//' @description
//' Performs a non-parametric bootstrap to estimate the distribution of the
//' loss probability \eqn{P(X < Y - \lambda)} across multiple outcome variables
...
//' Each element \code{(k, j)} represents the loss probability for the \eqn{j}-th
...
//' @export
// [[Rcpp::export]]
 NumericMatrix c_loss_boot_vec(IntegerVector xid,
                              NumericMatrix x_mat,
   ...
   for (int k = 0; k < nsim; ++k) {
     ...
     for (int j = 0; j < nj; ++j) {
       ...
       NumericVector loss_res = c_loss(outx[Range(0, nx_u - 1)],
